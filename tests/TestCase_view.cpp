...
TEST_CASE( "View external entity creation and matching", "[View]" )
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        components.add<Component_0>(e);
    }
    for (int i = 0; i < 20; i++) {
        auto e = entities.create();
        components.add<Component_0>(e);
        components.add<Component_1>(e);
    }
...
TEST_CASE( "View internal entity creation", "[View]" )
{
    ...
    view_factory.get(signature_0).add<Component_0>(entity_0);
    view_factory.get(signature_0).add<Component_1>(entity_0);
    view_factory.get(signature_0).get<Component_0>(entity_0).x = 42;
    view_factory.get(signature_0).get<Component_1>(entity_0).x = 13;

    int count = 0;
    for (auto e : view_factory.get(signature_0)) {
        count ++;
        REQUIRE( view_factory.get(signature_0).get<Component_0>(e).x == 42 );
    }
    REQUIRE( count == 1);
    ...
    for (auto e : view_factory.get(signature_1)) {
        count ++;
        REQUIRE( view_factory.get(signature_1).get<Component_0>(e).x == 42 );
        REQUIRE( view_factory.get(signature_1).get<Component_1>(e).x == 13 );
    }
    REQUIRE( count == 1);
}
...
TEST_CASE( "View mixed internal entity creation", "[View]" )
{
    ...
    view_factory.get(signature_0).add<Component_0>(entity_1);
    view_factory.get(signature_0).add<Component_1>(entity_1);
    view_factory.get(signature_0).get<Component_0>(entity_1).x = 42;
    view_factory.get(signature_0).get<Component_1>(entity_1).x = 13;
